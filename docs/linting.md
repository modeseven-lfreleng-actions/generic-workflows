...
thin caller in each consuming repository delegates to it on whichever
events that repository supports — pull requests where they exist, and
`push` for the Gerrit mirrors described below, which never see one.
...
That lookup cannot always reach a verdict. GitHub answers `404` for a
repository the token cannot read, so an organisation with **no**
`.github` repository and one with a **private** `.github` are
indistinguishable. The workflow confirms what it can — a readable
repository makes a missing file a fact — and warns when it cannot,
proceeding as though the organisation has no fallback so the
documented green skip survives. Set `org_fallback_required: true`
where the organisation knows its fallback exists, so an unreadable
one fails rather than passing a check that linted nothing.
...
implementation of prek's matching rather than two. That includes
matching a hook's `alias`, which prek's `--skip` accepts as readily
as an id, so an exclusion naming an alias behaves the same in every
mode.

A task that excludes every one of its hooks resolves to a job
reporting nothing to do.

### Which hook stages a run reaches
...
Nothing there says `commit-msg`; prek learns it after fetching the
manifest. Resolving that at plan time would mean cloning every remote
repository the configuration names, which is what the executor
stopped doing because a hook excluded for being unavailable then
...
### `prek.toml` is not supported

prek reads its own `prek.toml` in preference to either YAML spelling,
and says so when it finds both — a warning naming the two files and
then using the TOML one.

This workflow reads neither TOML nor a prek-native schema, so it
**refuses** a repository carrying one rather than guessing. Left
unnoticed, the two ways of meeting that file are both wrong: alone it
looks like a repository with no configuration and skips green, and
beside a YAML file it would lint the configuration prek ignores.

Use `.pre-commit-config.yaml`, or name a file explicitly with
`config_path`, which overrides discovery and takes the refusal with
it.
...
- **The resolver runs isolated (`python -I -`) under `--no-config`.**
  ...
  no hijack.
- **The job summary redacts `config_url`.** The accepted URL grammar
  admits a query string, which is how a private configuration
  commonly arrives — a presigned URL. The executor receives the URL
  whole; the summary, which anyone able to see the run can read,
  receives an origin and a path plus a note naming each part it
  dropped. The redirect refusal goes through the same redaction,
  since a target chosen by the server inherits whatever the caller's
  URL carried.
- **The planner rejects credentials in a URL.** `https://user:pass@host/...`
  validates against the grammar but cannot work: `urllib` leaves the
  pair in `Request.host` and builds no `Authorization` header, so the
  request goes to a host by that literal name. Rather than fail
  mid-download, the planner refuses it and says why. A presigned
  query string remains the supported route, and that one works.

  **A presigned URL is still not a secret-bearing one.** The plan job
  passes `config_url` to the lint job through the matrix, so the
  query string reaches `GITHUB_OUTPUT` and the workflow's `matrix`
  output, which anyone able to read the run can see. Redaction covers
  what this workflow *prints*, not what it must hand onward. Prefer a
  URL that needs no credential, pinned with `config_sha256`; a
  signature short-lived enough to expire before the log matters is
  the next best thing.
  lfreleng-actions/generic-workflows#52 tracks removing the URL from
  job outputs altogether.
...
`zizmor --persona=auditor` reports one class of finding, recorded
rather than claimed clean. A whole-repository auditor run reports
seven low-severity `self-repository` findings: four from callers that
predate this change, and three from `linting-action.yaml`'s self-test
jobs. That audit asks for GitHub's newer `uses: $/...` syntax in
place of `./...`, and actionlint blocks adoption: **actionlint
rejects `$/`** as a malformed reusable-workflow reference, so all
seven stay on `./` until actionlint learns it. Suppressing the
finding would hide a real improvement rather than a false positive,
and it stands for that reason.
...
The `warns` cells hold for a **trusted** event alone — a push, a
schedule, a manual dispatch, or a pull request whose head is this
same repository. Where the hooks come from a fork, an unmonitored
runner **fails under either policy**: the run would execute code
written by whoever opened the pull request with no egress record at
all, and on a self-hosted runner whatever it did outlives the job.
Withholding the token does not cover that, because the token is not
what is at risk.
