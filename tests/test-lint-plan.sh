...
# Assert on the log the last case wrote.
stdout_contains() {
  local desc="$1" needle="$2"
...
stdout_lacks() {
  ...
  if grep -q -- "${needle}" "${workdir}/stdout"; then
    failed=$((failed + 1))
    printf 'FAIL: %s (log unexpectedly has %s)\n' "${desc}" \
      "${needle}" >&2
    sed 's/^/  /' "${workdir}/stdout" >&2
    return 0
  fi
...
# Credentials in a URL are refused rather than carried. The grammar
# admits 'user:pass@host', but urllib leaves the pair in Request.host
# and builds no Authorization header:
#
#   >>> urllib.request.Request(
#   ...     'https://user:s3cret@example.org/c.yaml').host
#   'user:s3cret@example.org'
#
# so the request goes to a host by that literal name and fails to
# resolve. Such a URL could never have worked at plan time, and
# accepting one would turn a configuration mistake into a download
# failure partway through the plan. Refused before any request, so
# these cases touch no network.
reject 'userinfo in config_url' \
  INPUT_CONFIG_URL='https://user:s3cret@example.org/config.yaml'

# Asserting on the REASON, not just the refusal. Without the check
# the URL still fails -- urllib tries to resolve a host literally
# named 'user:s3cret@example.org' -- so a bare 'reject' passes either
# way and proves nothing. Mutation-testing caught exactly that.
stdout_contains 'it is refused for the credentials, not the DNS' \
  'credentials in a URL'

# And the secret does not reach the log while being refused.
stdout_lacks 'the refusal does not echo the secret' 's3cret'

reject 'username-only userinfo in config_url' \
  INPUT_CONFIG_URL='https://token@example.org/config.yaml'
stdout_contains 'username-only is refused the same way' \
  'credentials in a URL'

reject 'userinfo in a plan entry config_url' \
  INPUT_PLAN='[{"name":"a","config_url":"https://u:p@example.org/c.yaml"}]'
stdout_contains 'a plan entry is refused the same way' \
  'credentials in a URL'
...
# A payload that was merely large but malformed would be rejected
...
# --- Hook stages a run cannot reach ----------------------------------
#
# prek runs pre-commit hooks, and manual ones when named; the other
# nine stages it passes over without output at exit 0. So a
# configuration declaring 'stages: [commit-msg]' -- gitlint being the
# common one -- gets a green check that never touched it, and the
# run-all default reports 'all hooks' while doing so.
#
# The resolver cannot fix that; the executor decides what it ran, and
# standalone-linting-action#156 tracks it. What the resolver can do is
# refuse to let it pass unmentioned.

stage_dir="${workdir}/stagecfg"
rm -rf "${stage_dir}"
mkdir -p "${stage_dir}"
...
accept 'a configuration with a commit-msg hook still plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'

# Named, so a reader of the run knows which hook went unrun rather
# than being told something vague about stages.
...
stdout_lacks 'a runnable hook is not named' 'ruff ('

# 'manual' hooks DO run when a task names them, so they are not
# unreachable in that mode and must stay out of the warning.
printf '%s\n' 'repos:
  - repo: local
    hooks:
      - id: manual-hook
...
accept 'a manual-stage hook plans under ci_skipped' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml' \
  INPUT_HOOKS='manual-hook'
stdout_lacks 'a named manual hook raises no warning' '::warning::'

# The same hook under the RUN-ALL default, which invokes prek
# unqualified -- and that leaves manual hooks alone. One hook, two
# answers depending on how the task invokes prek, so a single
# runnable set for both modes gets one of them wrong.
accept 'a manual-stage hook plans under run-all' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_contains 'run-all warns about a manual hook' 'manual-hook'

# 'default_stages' supplies the stages of every hook that declares
# none, so reading an absent 'stages' as 'every stage' misses a
# configuration that pins them all away from pre-commit.
printf '%s\n' 'default_stages: [commit-msg]
repos:
  - repo: local
    hooks:
      - id: inherits-default
        name: inherits the default stage
        entry: "true"
        language: system' \
  > "${stage_dir}/.pre-commit-config.yaml"

accept 'a configuration using default_stages plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_contains 'the inherited stage is noticed' 'inherits-default'

# An explicit 'stages' overrides the default, so this one runs and
  ...
  - repo: local
    hooks:
      - id: overrides-default
        name: overrides the default stage
        entry: "true"
        language: system
        stages: [pre-commit]' \
  > "${stage_dir}/.pre-commit-config.yaml"

accept 'an explicit stage overrides default_stages' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_lacks 'the overriding hook raises no warning' '::warning::'

# An EMPTY list is not 'no stages': pre-commit merges with 'hook
# stages or default_stages', and [] is falsey there, so it takes the
# default. Reading [] as unrestricted would stay silent about a hook
# the default pins to commit-msg.
printf '%s\n' 'default_stages: [commit-msg]
repos:
  - repo: local
    hooks:
...
accept 'an empty stages list plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_contains 'an empty list falls back to the default' \
  'empty-stages'

# With no default_stages to fall back to, an empty list leaves the
# hook unrestricted -- pre-commit's own default is every stage.
printf '%s\n' 'repos:
  - repo: local
    hooks:
      - id: empty-no-default
        name: empty stages, no default
        entry: "true"
        language: system
        stages: []' \
  > "${stage_dir}/.pre-commit-config.yaml"

accept 'an empty stages list without a default plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_lacks 'it raises no warning' '::warning::'

# An explicitly EMPTY default_stages is a declared empty set, not an
# absent key: such a hook runs at no stage whatsoever. Collapsing it
# to 'not declared' would read a hook that can never run as one that
# always does.
printf '%s\n' 'default_stages: []
repos:
  - repo: local
...
accept 'an empty default_stages plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_contains 'a hook with no stage at all is reported' \
  'never-runs'

# A hook id is fork-controlled on a pull request, and a newline in it
# would close the ::warning:: line and open whatever followed as a
# fresh workflow command.
printf '%s\n' 'repos:
  - repo: local
    hooks:
      - id: "evil\nzzinjected"
...
accept 'a hook id carrying a newline still plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'

# The injected text must not begin a line of its own: that is what
# would let it be read as a workflow command.
stdout_lacks 'no line starts with the injected text' '^zzinjected'

# A hook listing BOTH reaches the run through pre-commit, so it is
# not unreachable either. Testing the intersection rather than the
# first element: a check reading stages[0] would report this one.
printf '%s\n' 'repos:
  - repo: local
    hooks:
      - id: both-stages
...
accept 'a hook listing a reachable stage among others plans' \
  GITHUB_WORKSPACE="${stage_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_lacks 'it raises no warning' '::warning::'

# --- display_config ---------------------------------------------------
#
# The job summary is readable by anyone who can see the run, and the
# accepted URL grammar admits userinfo and a query string -- which is
# how a private configuration is usually served, by a presigned URL or
# an embedded token. Writing the URL whole would publish them.
#
# Extracted and called directly: reaching this end to end needs a live
# HTTPS server, and the resolver accepts nothing else.

check_redaction() {
  local desc="$1" url="$2" expect="$3" got

  got="$("${PY_RUN[@]}" - "${resolver}" "${url}" <<'PYEOF'
import ast
import sys
import urllib.parse
...
wanted = {"display_config", "redact_url"}
found = set()
for node in ast.parse(source).body:
    if isinstance(node, ast.FunctionDef) and node.name in wanted:
        module = ast.Module(body=[node], type_ignores=[])
        exec(compile(module, "<display_config>", "exec"), namespace)
        found.add(node.name)
missing = sorted(wanted - found)
if missing:
    sys.exit(f"resolver is missing: {missing}")
print(
    namespace["display_config"](
        {"kind": "url", "config": sys.argv[2]}
    )
)
PYEOF
)"
...
check_redaction 'a plain URL is shown whole' \
  'https://example.org/linting/cfg.yaml' \
  'https://example.org/linting/cfg.yaml'

# The two that carry credentials in practice.
check_redaction 'userinfo is removed' \
  'https://user:s3cret@example.org/cfg.yaml' \
  'https://example.org/cfg.yaml (userinfo redacted)'

check_redaction 'a presigned query is removed' \
  'https://example.org/cfg.yaml?X-Amz-Signature=deadbeef' \
  'https://example.org/cfg.yaml (query redacted)'

check_redaction 'both are removed and both are named' \
  'https://u:p@example.org/cfg.yaml?sig=x#frag' \
  'https://example.org/cfg.yaml (userinfo/query/fragment redacted)'

# A port is part of the origin, not a secret, so it survives.
check_redaction 'a port survives' \
  'https://example.org:8443/cfg.yaml' \
  'https://example.org:8443/cfg.yaml'

# A parse failure reports PyYAML's own diagnostic, and that quotes
# the offending source across several lines. Emitted raw it would
# spill out of the '::error::' annotation, and every line after the
# first would be read by the runner as ordinary output -- or as
# another workflow command, for a configuration crafted to put one
# at column zero.
bad_dir="${workdir}/badyaml"
rm -rf "${bad_dir}"
mkdir -p "${bad_dir}"
printf '%s\n' 'repos: [' > "${bad_dir}/.pre-commit-config.yaml"

reject 'a configuration that does not parse' \
  GITHUB_WORKSPACE="${bad_dir}" PRIMARY='.pre-commit-config.yaml'

# PyYAML's diagnostic runs to three lines. Sanitised, all of it sits
# on the single '::error::' line; raw, the continuations begin lines
# of their own, which is what lets a crafted configuration place a
# workflow command at column zero.
stdout_lacks 'the parse error does not spill across lines' \
  '^expected the node content'

# A plan entry reaching a configuration whose containers are wrong.
bad_shape_dir="${workdir}/badshape"
rm -rf "${bad_shape_dir}"
mkdir -p "${bad_shape_dir}"
printf '%s\n' 'repos: [just-a-string]' \
  > "${bad_shape_dir}/.pre-commit-config.yaml"

reject 'a malformed repos list in a plan entry' \
  INPUT_PLAN='[{"name":"a"}]' \
  GITHUB_WORKSPACE="${bad_shape_dir}" PRIMARY='.pre-commit-config.yaml'

# The resolver's own diagnosis, not a traceback from inside the stage
# check. That check walks 'repos[].hooks[]' assuming mappings, and the
# plan path used to reach it before anything validated the shape.
stdout_contains 'it names the offending value' \
  'is not a mapping'
stdout_lacks 'it does not raise' 'AttributeError'

# A repository entry with no 'repo' key. prek refuses the whole
# configuration for this -- 'missing field `repo`' -- but the
# resolver used to walk past it, find no hook ids, and read the
# result as a deliberately empty configuration: zero tasks, green
# check, nothing linted, over a file prek would not have loaded.
no_repo_dir="${workdir}/norepo"
rm -rf "${no_repo_dir}"
mkdir -p "${no_repo_dir}"
printf '%s\n' 'repos: [{hooks: []}]' \
  > "${no_repo_dir}/.pre-commit-config.yaml"

reject 'a repository entry with no repo key' \
  GITHUB_WORKSPACE="${no_repo_dir}" PRIMARY='.pre-commit-config.yaml'
stdout_contains 'it names the missing field' "non-empty string 'repo'"

# The same shape reached through a plan entry.
reject 'a repo-less entry through a plan entry' \
  INPUT_PLAN='[{"name":"a"}]' \
  GITHUB_WORKSPACE="${no_repo_dir}" PRIMARY='.pre-commit-config.yaml'

# A non-string 'repo' is refused too: coercing it would accept
# 'repo: 3' as a valid source.
printf '%s\n' 'repos: [{repo: 3, hooks: []}]' \
  > "${no_repo_dir}/.pre-commit-config.yaml"
reject 'a non-string repo value' \
  GITHUB_WORKSPACE="${no_repo_dir}" PRIMARY='.pre-commit-config.yaml'

# An empty 'repos' list stays valid: that says 'no hooks' on purpose,
# and is the case the new check must not sweep up.
empty_repos_dir="${workdir}/emptyrepos"
rm -rf "${empty_repos_dir}"
mkdir -p "${empty_repos_dir}"
printf '%s\n' 'repos: []' \
  > "${empty_repos_dir}/.pre-commit-config.yaml"
accept 'an explicitly empty repos list still plans' \
  GITHUB_WORKSPACE="${empty_repos_dir}" \
  PRIMARY='.pre-commit-config.yaml'
